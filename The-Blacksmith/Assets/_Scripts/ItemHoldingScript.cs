...
public class ItemHoldingScript : MonoBehaviour {
	...
	void Update () {
		...
		if(shackle != null){
			if(shackle.gameObject.tag == "Shop Box"){
				if(Input.GetKeyDown ("[1]")){
					if(myPlayerState.points >= 30){
						shackle.gameObject.GetComponent<DepositStates>().inst(1);
				...
				if(Input.GetKeyDown ("[3]")){
					if(myPlayerState.points >= 20){
						shackle.gameObject.GetComponent<DepositStates>().inst(2);
				...
				if(Input.GetKeyDown ("[4]")){
					if(myPlayerState.points >= 80){
						shackle.gameObject.GetComponent<DepositStates>().inst(3);
		...
		if(Input.GetMouseButtonDown(1)){
			...
			if(rightHand != null){
				...
				if(rightHand == null){
					if(hit.collider.tag=="Iron" || hit.collider.tag == "Steel"){//Right now we only pick up barrels, but we can do SO MUCH MORE!
			...
			if(!locked){
				...
				if(shackle.tag=="Anvil"){
					...
					if(rightHand.tag == "Shield")
						rightHand.GetComponent<ItemStates>().setstate(0);
					rightHand.GetComponent<ItemStates>().addQuality (shackle.GetComponent<WorkbenchStates>().qual);
					shackle=null;
	...
	void updateHUD(){
		...
		if(rightHand != null){
			...
			if(rightHand.tag == "Weapon" || rightHand.tag == "Iron" || rightHand.tag == "Steel"){
		...
		PointText.text = "Coins: " + myPlayerState.points.ToString ();
	}
}
